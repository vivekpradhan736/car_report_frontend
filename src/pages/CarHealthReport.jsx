...
import { CarHealthReportPDF } from "../components/CarHealthReportPDF";
...
const CarHealthReport = () => {
  ...
  return (
    <Card className="shadow-lg border-0 grid gap-8 lg:grid-cols-2">
      <div className="flex justify-center items-center">
          <CardContent className="p-0 w-full max-w-4xl">
            {/* Header */}
            <div style={{ backgroundColor: "black" }} className=" text-white p-4 rounded-t-lg flex justify-between items-center text-sm">
              <img
                              src="/logo_mwg.png"
    ...
    {data.serviceRecommendations && data.serviceRecommendations.length > 0 && (
      ...
      {data.serviceRecommendations?.map((rec, index) => (
        ...
        >
          ...
          <PDFDownloadLink
                              document={<CarHealthReportPDF data={data} />}
                              fileName={`car-health-report-${data.ownerName.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}.pdf`}
...
export default CarHealthReport;
